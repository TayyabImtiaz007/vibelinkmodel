...
# response = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=f'{folder_name}/data.txt')
# if 'Contents' in response:
def update_file_s3_bucket(bucket_name, customerdata, file_path, additional_text):
        # Download data.txt from S3
        s3_client.download_file(bucket_name, customerdata, file_path)

        # Read text from another .txt file
        # with open(customerdata, 'r') as file:
        #     additional_text = file.read()

        # Append the additional text to data.txt
        with open(file_path, 'a+', newline='') as file:#, encoding='utf-8'
            file.write(additional_text)

        # Upload modified data.txt back to S3
        # s3_client.upload_file(file_path, bucket_name, customerdata)
        print("File updated and uploaded successfully.")

        # Clean up downloaded file
        # os.remove('data.txt')
    # else:
    #     print("data.txt does not exist in the folder in the bucket.")